...
// here
let pumpkintwo = document.getElementById("pumpkin-2");
...
function updateBroom() {
    // horizontal moves
    let diffX = mousePosX - broom.offsetLeft - (broom.offsetWidth/1);
...
function updatePumpkintwo() {
    let targetPumpkinX = pumpkintwo.offsetLeft - 5;
    if (targetPumpkinX < -pumpkintwo.offsetWidth) {
        targetPumpkinX = container.offsetWidth;
        pumpkintwo.style.top = (Math.random() * 400) + "px";
    }
    pumpkintwo.style.left = targetPumpkinX + "px";
}
...
function checkForItemCollect() {
    ...
    let diffXtwo = (pumpkintwo.offsetLeft + (pumpkintwo.offsetWidth/2)) - (broom.offsetLeft + (broom.offsetWidth/2));
    //console.log(diffX);
    
    let diffYtwo = (pumpkintwo.offsetTop + (pumpkintwo.offsetHeight/2)) - (broom.offsetTop + (broom.offsetHeight/2));
    //console.log(diffY);
    ...
    if (Math.abs(diffXtwo) < 40 && Math.abs(diffYtwo) < 40) {
        //console.log("collect item");
        pumpkintwo.style.left = container.offsetWidth + "px";
        pumpkintwo.style.top = (Math.random() * 400) + "px";
